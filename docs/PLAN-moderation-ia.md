...
- **Cron toutes les 3 h actif** : `wrangler deploy` publie le script et le trigger
  `0 */3 * * *`. Le sous-domaine workers.dev a été créé une fois côté Cloudflare, puis
  le déploiement Worker est automatisé par GitHub Actions.
...
- `worker/src/index.ts` : endpoints `/_moderation/scan` + `/_moderation/callback` + cron toutes les 3 h
  (`wrangler.jsonc`).
...
- **Cron toutes les 3 h** (Worker scheduled) : balayage de fond de tous les nouveaux médias, **même en
...
- Il sera scanné au prochain cron toutes les 3 h (ou via un signal de publication si le user republie) ; au
...
- **Bouton « Lancer un scan maintenant » en haut de la vue** : déclenche un scan à l'appui
  ...
  pour tester sans attendre le cron toutes les 3 h.
...
- `action: 'scan-media'` → **branché sur le bouton « Lancer un scan maintenant »** (2.1) : signale au
  videur de lancer un lot de scan immédiat ; le mode normal reste le cron toutes les 3 h. Renvoie un état
...
2. Videur : appels Sightengine (upload binaire image + Upload API vidéo) + état R2
   (`media-scanned.json` / `media-moderation.json`) + compteur d'ops (1.6) + cron toutes les 3 h + endpoint
